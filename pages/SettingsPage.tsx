...
export const SettingsPage: React.FC<SettingsPageProps> = ({ onBack, darkMode, onToggleDarkMode }) => {
    ...
    React.useEffect(() => {
        // We now ignore the local setting as requested by admin, but keeping state for logic if needed elsewhere
        setNightlifeEnabled(true);
    }, []);

    const toggleNightlife = async () => {
        // Disabled UI, but keeping function for logic consistency if called
        console.log('Notification toggles are now managed by administrator.');
    };
