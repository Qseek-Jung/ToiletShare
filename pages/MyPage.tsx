...
const MyPage: React.FC<MyPageProps> = ({
    ...
    useEffect(() => {
        ...
        loadReviews();
    }, [user.id, activeTab, refreshTrigger, Array.from(bookmarks).join(',')]); // Fix: Use stable primitive dependency instead of Set reference
    ...
    return (
        <TextLayout className="p-0" noPadding>
            ...
            {/* Fixed Header */}
            <div className="absolute top-0 left-0 right-0 z-20 bg-surface dark:bg-surface-dark border-b border-border dark:border-border-dark pt-[env(safe-area-inset-top)] shadow-sm">
                ...
                {/* Rolling Notice Bar */}
                {appNotices.length > 0 && (
                    <div className="bg-white dark:bg-gray-800 border-t border-border/50 dark:border-border-dark/50 h-10 flex items-center px-4 relative z-10 transition-colors">
                        ...
                        <button
                            onClick={() => window.location.hash = `#/notice/${appNotices[currentNoticeIndex].id}`}
            ...
            {/* Scrollable Content */}
            {/* Height calculation: Header (roughly 60px) + Notice Bar? (40px)
                If Notice bar exists, top padding needs to be larger.
                However, dynamic height is tricky with CSS only.
                Simplest fix: Use a ref to measure header height or extensive padding.
                Or, since we know the exact height: 60px + (notices ? 40px : 0px).
                Actually, simpler: put fixed header outside, and this div pushes itself down?
                No, because we are inside TextLayout absolute inset.
                Let's use Safe Area + Fixed Pixel height for padding-top.
            */}
            <div
                className={`absolute inset-0 overflow-y-auto overflow-x-hidden pb-[env(safe-area-inset-bottom)] scrollbar-hide`}
                style={{
                    paddingTop: `calc(env(safe-area-inset-top) + ${appNotices.length > 0 ? '106px' : '66px'})`,
