...
serve(async (req) => {
    ...
    try {
        ...
        if (!token) {
            // Not a failure! We already have the DB record for the in-app notification center.
            // Just mark it as 'in_app_only' so the UI knows it wasn't pushed to a device.
            ...
            return new Response(JSON.stringify({
                success: true,
                inAppOnly: true,
                message: "No push token, delivered in-app only"
            }), {
                status: 200,
