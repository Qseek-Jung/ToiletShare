...
class NotificationService {
    ...
    public async initialize() {
        ...
        try {
            if (Capacitor.getPlatform() === 'web') return; // Skip on web
    ...
    public async scheduleReviewReminder(toiletId: string, toiletName: string) {
        const REVIEW_REMINDER_ID = 10001; // Fixed ID to prevent duplicates (last one wins)
        ...
        try {
            ...
            await LocalNotifications.schedule({
                notifications: [{
                    ...
                    title: '리뷰 남기기',
                    body: msg,
                    schedule: { at: new Date(Date.now() + 5 * 60 * 1000) }, // 5 mins (Matches Admin UI)
    ...
    public async cancelReviewReminder(toiletId: string) {
        const REVIEW_REMINDER_ID = 10001;
        try {
    ...
    public async configureNightlifeReminder() {
        ...
        try {
            // 0. Check User Preference (DEPRECATED: Now Admin Driven)
